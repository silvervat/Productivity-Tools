...
function flattenObject(obj: any): Record<string, string> {
  ...
  if (obj?.guid) result["GUID"] = String(obj.guid);
  if (obj?.product?.name) result["Product.Name"] = String(obj.product.name);
  if (obj?.product?.description) result["Product.Description"] = String(obj.product.description);
  if (obj?.product?.type) result["Product.Type"] = String(obj.product.type);
  ...
  const discoverFields = useCallback(async () => {
    ...
    try {
      ...
      for (const selectionItem of selection) {
        ...
        try {
          // Primary method: Get full object properties using correct API
          ...
          if (fullProperties && Array.isArray(fullProperties) && fullProperties.length > 0) {
            const firstProps = fullProperties[0];
            const flattened = flattenObject(firstProps);
            ...
            Object.entries(flattened).forEach(([key, value]) => {
              if (value && value.trim().length > 0 && !fieldsMap[key]) {
                fieldsMap[key] = {
                  ...
                  selected: fieldCount < 8, // Auto-select first 8 fields
                };
                fieldCount++;
              }
            });
          } else if (fullProperties && !Array.isArray(fullProperties)) {
            // Handle non-array response
            const flattened = flattenObject(fullProperties);
            Object.entries(flattened).forEach(([key, value]) => {
              if (value && value.trim().length > 0 && !fieldsMap[key]) {
                fieldsMap[key] = {
                  name: key,
          ...
          // Fallback 1: Try getObjects for basic properties
          try {
            ...
            if (Array.isArray(objects) && objects.length > 0) {
              ...
              const basicProps = ['name', 'type', 'guid', 'code', 'description', 'id'];
            ...
            console.warn(`Fallback getObjects failed:`, e);
          }
        }
      }
      ...
      if (fieldCount === 0) {
        ...
        setDiscoveryError("⚠️ Kasutades vaikimisi väljade komplekti");
      }

      setDiscoveredFields(fieldsMap);
      if (fieldCount > 0 && !discoveryError) {
      ...
      for (const selectionItem of selection) {
        ...
        try {
          ...
          for (let idx = 0; idx < objectRuntimeIds.length; idx++) {
            ...
            const flattened = flattenObject(props);
